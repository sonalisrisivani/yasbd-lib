import re

from yasbd.rules.base import Rules


# fmt: off
class RuRules(Rules):


    TITLE_ABBRVS = Rules.TITLE_ABBRVS | {
        # Standard Professional
        "ак", "акад", "доц", "проф", "д-р", "канд", "арх", "зав", "зам", "корр", "ред",
        "ср", "нач", "преп", "асс", "инж", "техн", "дир", "зам. дир", "зав. каф", "асп",
        "в.н.с", "гл. науч. сотр", "вед. науч. сотр", "ст. науч. сотр", "науч. сотр",
        "мл. науч. сотр", "м.н.с", "чл.-к", "чл.-корр", "чл.-кор", "к.м.с",

        # Military / Political / Administrative
        "ген", "полк", "подп", "лейт", "кап", "и.о",

        # Noble / Royal
        "кн", "вел. кн", "гр", "бар", "св.кн", "имп",
        "имп-ца", "цес", "цес-на",

        # Others
        "г-н", "г-жа", "госп", "тов", "св", "бл", "им", "пол", "ст", "трад", "букв", "о", "оо",
        "лат", "венг", "исп", "кат", "укр", "нем", "англ", "фр", "итал", "греч", "евр",
        "араб", "яп", "слав", "кит", "рус", "русск", "латв", "словацк", "хорв",
    }

    # English geopolitical abbreviations (USA, UK, NATO, etc.) may appear in
    # Russian texts but are typically replaced by Russian equivalents or treated
    # as foreign Latin-script tokens rather than native patterns.
    DOTTED_GEOPOL_ABBRVS = set()

    REFERENCE_ABBRVS = Rules.REFERENCE_ABBRVS | {
        "см", "ср", "цит", "цит по", "с", "стр", "гл", "разд", "раздл", "§",
        "п", "пп", "ст", "табл", "рис", "прим", "прил", "напр", "вып", "т.к", "и.о", "ч",
        "чч", "абз", "пт", "доп", "сокр", "искл", "изд", "собр", "соч", "т", "тт", "автореф",
        "анот", "реф", "рец", "chap", "no", "и др", "и пр", "и т.п", "в т.ч",
    }

    SECTION_MARKERS = Rules.SECTION_MARKERS | {
        "Глава", "Часть", "Раздел", "Статья", "Параграф",
        "Том", "Книга", "Подраздел", "Модуль", "Дивизион",
        "Использование", "Единица", "Предисловие", "Введение"
    }

    INLINE_ONLY_ABBRVS = Rules.INLINE_ONLY_ABBRVS | {
        "т.е", "т.к", "т.н", "в", "вв", "г", "гг", "до н.э", "н.э", "руб", "коп", "долл",
        "тыс", "млн", "млрд", "трлн", "экз", "ед", "шт", "ул", "пр", "пер", "пл",
        "обл", "р-н", "окт", "нояб", "дек", "янв", "февр", "март", "апр",
        "июн", "июл", "авг", "сент",
    }

    NAMES_WITH_EXCLAMATION = Rules.NAMES_WITH_EXCLAMATION | {
        "Яндекс", "Билайн", "Сбер",
    }

    DATE_ABBRVS = Rules.DATE_ABBRVS | {
        # Months
        "янв", "фев", "мар", "апр", "июн", "июл", "авг", "сен",
        "окт", "ноя", "дек",

        # Day
        "пн", "вт", "ср", "чт", "пт", "сб", "вс",
    }

    COMMON_SENT_STARTERS = {
        # Pronouns
        "Я", "Мы", "Вы", "Он", "Она", "Оно", "Они", "Это", "Тот",
        "Эти", "Те", "Там",

        # Question words
        "Где", "Зачем", "Как", "Какой", "Когда", "Кого", "Кому",
        "Кто", "Почему", "Чей", "Что",

        # Adverbs / Transitions
        "Более", "В-третьих", "Во-вторых", "Во-первых",
        "Впоследствии", "Впрочем", "Далее", "Затем",
        "Изначально", "Иначе", "Кроме", "Между", "Наконец",
        "Не менее", "Однако", "Позднее", "Позже", "Последний",
        "Потом", "Поэтому", "Прежде", "Ранее", "Раньше",
        "Сейчас", "Следовательно", "Следующий", "Тем временем",
        "Тем", "Теперь", "Тогда", "Того", "Тому",

        # Other common starters
        "Делают", "Сделал", "Миллионы",
    }

    # Particles like "мол", "де", "дескать" exist in Russian but are rarely
    # used alongside quoted text, so they don't affect sentence boundaries.
    POST_QUOTATIVE_PARTICLES = set()

    # fmt: on
    @classmethod
    def _compile_regex_dynamically(cls):
        super()._compile_regex_dynamically()
        cls.MID_SENTENCE_FINDER_LST.append(
            re.compile(r"(?<=\.)\s*[а-яё]\.|[а-яё]\.(?=\s+.\.)", re.I)
        )
